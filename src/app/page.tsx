...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans selection:bg-brand-100 selection:text-brand-900">
      ...
      <section className="pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden bg-gradient-to-b from-gray-50 to-white relative">
        {/* Background Elements */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-gray-200 shadow-sm text-brand-700 text-sm font-bold mb-8 animate-fade-in-up">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-brand-600"></span>
            </span>
            Nouveau au Bénin : La vente automatisée
          </div>
          
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black tracking-tight text-gray-900 mb-8 leading-tight">
            Vendez vos tickets WiFi <br className="hidden md:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-600 to-blue-600">
              Même quand vous dormez.
            </span>
          </h1>
          
          <p className="text-lg sm:text-xl text-gray-500 max-w-3xl mx-auto mb-12 leading-relaxed">
            Kodfi connecte votre routeur Mikrotik au Mobile Money.
            Fini les tickets papiers, fini la monnaie, fini les vols.
            Gérez votre business de WiFi Zone comme un pro, depuis votre téléphone.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Link href="/signin" className="w-full sm:w-auto">
              <button className="w-full px-8 py-4 bg-brand-600 text-white rounded-xl font-bold text-lg hover:bg-brand-700 transition shadow-xl shadow-brand-200 flex items-center justify-center gap-2">
                Commencer Gratuitement <ArrowRight size={20} />
              </button>
            </Link>
            <Link href="#how-it-works" className="w-full sm:w-auto">
              <button className="w-full px-8 py-4 bg-white text-gray-700 border border-gray-200 rounded-xl font-bold text-lg hover:bg-gray-50 transition flex items-center justify-center gap-2">
                Voir la Démo
              </button>
            </Link>
          </div>
          ...
          <div className="mt-12 flex items-center justify-center gap-8 text-gray-400 grayscale opacity-70">
             {/* Logos factices pour la preuve sociale */}
