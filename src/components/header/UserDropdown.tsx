...
export default function UserDropdown() {
  ...
  // 👇 1. On récupère aussi le 'status'
  const { data: session, status } = useSession(); 

  // 👇 2. GESTION DU CHARGEMENT (Skeleton)
  // Si c'est en train de charger, on affiche un rond gris au lieu de "undefined"
  if (status === "loading") {
    return (
      <div className="flex items-center gap-3">
        <div className="h-11 w-11 animate-pulse rounded-full bg-gray-200 dark:bg-gray-700" />
        <div className="hidden h-4 w-24 animate-pulse rounded bg-gray-200 dark:bg-gray-700 lg:block" />
      </div>
    );
  }

  // 3. Préparation des données (avec sécurité anti-undefined)
  const user = {
    firstName: session?.user?.firstname || session?.user?.name?.split(" ")[0] || "Compte",
    fullName: session?.user?.name || "Utilisateur",
    email: session?.user?.email || "",
    avatar: session?.user?.image || "/images/user/owner.png",
  };
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 border border-gray-100 dark:border-gray-700">
          <Image
            width={44}
            height={44}
            src={user.avatar}
      ...
      >
        ...
        <button
          ...
          className={`${itemClasses} mt-3 w-full text-left`}
        >
          <Icons.Logout />
          Se déconnecter
        </button>
...
// --- Styles & Icons ---
const itemClasses = "flex items-center gap-3 px-3 py-2 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300";
